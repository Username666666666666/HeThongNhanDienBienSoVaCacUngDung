...
/**
 * Generic wrapper để handle Supabase queries với error handling tập trung
 */
...
export const fetchRecord = async <T,>(
  ...
  return executeQuery(() =>
    supabase.from(table).select('*').match(filters).maybeSingle()
  );
};

/**
 * Fetch nhiều bản ghi từ bảng
 */
